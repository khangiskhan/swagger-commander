...
function setupSubcommand(client, resourceName) {
    apiClient = client;
    var apiObj = apiClient[resourceName];
    ...
    var subCommandArg = process.argv[3];
    if (!subCommandArg) {
...
function runAction(cmdAction, argumentNames, optionKeys) {
    ...
    for (var i = 0; i < requiredArgumentValues.length; i++) {
        var key = argumentNames[i].name;
        logger.debug(requiredArgumentValues[i]);
        // if argument type is "body", then parse JSON
        requiredArguments[key] = (argumentNames[i].type === 'body') ?
                                    parseBody(requiredArgumentValues[i]) : requiredArgumentValues[i];
    }
    ...
    if (auth) {
        ...
        _.each(auth, function (authObj, authName) {
            apiClient.clientAuthorizations.add(authName, authObj);
            logger.debug('Calling action with auth: ', util.inspect(authObj));
        });
    }
